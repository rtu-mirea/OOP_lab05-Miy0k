...
public class DistanceExaminator implements Serializable {
    ...
    public void changeQuestion(int i, String question, String answer) throws Exception{
        if (question.compareTo("") == 0 || answer.compareTo("") == 0)
            throw new Exception("Введите вопрос, а затем ответ на него.");
        if (curreantUser == -1) {
    ...
    public void examination()throws Exception{
        ...
        if (curreantUser < 0) throw new Exception("Вход в систему не выполнен.");
        ...
        JOptionPane.showMessageDialog(new JFrame(), "Начало экзамена");
        for (int i = 0; i < 5; i++){
            flag = true;
    ...
    public int getMark()throws Exception{
        if (curreantUser < 0) throw new Exception("Вы не вошли в систему или не имеете доступа к меню");
